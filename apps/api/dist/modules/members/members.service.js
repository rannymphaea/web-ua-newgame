...
let MembersService = MembersService_1 = class MembersService {
    ...
    async list(opts) {
        try {
            ...
            if (opts.division)
                q = q.where('division', '==', opts.division);
            if (opts.role)
    ...
    async getOne(uid) {
        try {
            ...
            if (!snap.exists)
                throw new common_1.NotFoundException(`Member ${uid} not found`);
    ...
    async create(dto, createdBy) {
        try {
            ...
            if (!dup.empty)
                throw new common_1.ConflictException(`Email ${dto.email} already exists`);
    ...
    async update(uid, dto, updatedBy) {
        try {
            ...
            await Promise.all([
                this.history.write({ userId: uid, changedBy: updatedBy, action: 'update_member', before, after, changedFields: (0, hash_util_1.diffKeys)(before, after) }),
    ...
    async import(format, data, importedBy) {
        try {
            const db = this.firebase.firestore;
            const members = [];
            if (format === 'json') {
            ...
            throw new common_1.BadRequestException(`Import failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
};
